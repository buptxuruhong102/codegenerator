var demoDemo = function () {
    var demoDemo = {
        ...
        save: function(){
            ...
            $.ajax({
                "type" : "POST",
                "url" : $.ctx + "/demoDemo/save",
                "data": $("#commentForm").serialize(),
                success: function (result) {
                    if(result.status == 200){
                        $('#commonModal').modal("hide");
                        demoDemo.tableService.ajax.reload();
                    }else{
         ...
         */
        edit: function (id) {
            var url = $.ctx + "/demoDemo/edit?id=" + id;
            $('#commonModal').modal("show").load(url);
        },
        /**
         * 删除数据源
...
$(function(){
    var dataTableServiceObj = {
        ...
        "columnDefs" : [
            ...
            },
            {
                ...
                "sWidth": "15%",
                "render" : function(data, type, now) {
                    var str ='<a href="###" onclick="demoDemo.edit('+ now.id +');">修改</a>'
                            +'&nbsp;&nbsp;<a href="###" onclick="demoDemo.del('+ now.id +');">删除</a>';
                    return str;
                }
            }
        ],
    ...
    //表格初始化
    demoDemo.tableService = CommonUtil.initTable(demoDemo.tableService, $("#tableService"), dataTableServiceObj);
